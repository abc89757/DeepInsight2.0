...
class ReportWriterNode(AgentNode):
    ...
    system_prompt = """
你是结构化数据分析报告撰写人。
请根据用户问题和报告上下文生成一份 Markdown 结构化数据分析报告。
报告上下文中每一轮包含分析目标、分析结果、数据限制和分析限制。
...
""".strip()
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report_body, report_thought = split_think_content(raw_report)
        report = self.clean_report(report_body)
        return self.write_report(state, report, report_thought=report_thought)

    def build_prompt(self, state: Dict[str, Any]) -> str:
        """构造最终报告撰写 prompt。

        输入:
            state: 当前图状态；包含用户问题、报告上下文和 Skill 报告模板。

        输出:
            要求模型输出 Markdown 报告的 prompt 字符串。
        """
        report_context = self.build_report_context(state)
        return f"""
用户问题：
{state["question"]}

报告上下文：
{json_dumps(report_context)}

请输出 Markdown 报告：
""".strip()

    def build_report_context(self, state: Dict[str, Any]) -> list[Dict[str, Any]]:
        """构造给报告撰写人使用的精简分析上下文。

        输入:
            state: 当前图状态；包含 `analysis_rounds`。
        输出:
            去掉 evidence_plan / evidence_result 后的报告上下文列表。
        """
        context: list[Dict[str, Any]] = []
        for item in state.get("analysis_rounds", []) or []:
            if not isinstance(item, dict):
                continue
            context.append(
                {
                    "round": item.get("round"),
                    "analysis_goal": item.get("analysis_goal", ""),
                    "analysis_result": item.get("analysis_result", ""),
                    "data_issue": self.clean_report_issue(item.get("data_issue", "")),
                    "analysis_issue": item.get("analysis_issue", ""),
                }
            )
        return context

    def clean_report_issue(self, issue: Any) -> str:
        """清理报告上下文中的限制说明，避免混入整段证据结果。"""
        text = str(issue or "").strip()
        if not text:
            return ""
        for marker in ["【数据情况】", "【证据结果】", "【初步含义】"]:
            if marker in text:
    ...
    def get_chart_artifacts_text(self, state: Dict[str, Any]) -> str:
        ...
        输出:
            给报告生成器使用的图表说明文本。
        """
        artifacts = state.get("chart_artifacts", []) or []
        return (
            "可引用的图表产物：\n"
            f"{json_dumps(artifacts)}\n\n"
            "如果存在可引用的图表文件，则报告必须在相关结论附近用 Markdown 图片语法引用，"
            "并在图下写 caption。只能引用上面 chart_artifacts 中真实存在的 file_path，"
            "不要从其他文本中复制或改写图片路径。"
        )
...
""".strip()
    ...
    def clean_report(self, report: str) -> str:
        ...
        输出:
            清理后的 Markdown 报告正文。
        """
        text, _ = split_think_content(report)
        text = text.strip()
    ...
    def write_report(self, state: Dict[str, Any], report: str, report_thought: str = "") -> Dict[str, str]:
        """把报告和元数据写入当前任务输出目录。
    ...
    def step_output(self, output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "thought_summary": output.get("report_thought"),
        }
